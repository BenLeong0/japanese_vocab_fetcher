...
def test_main(monkeypatch, test_dict: FullTestDict):
    ...
    monkeypatch.setattr(
        "requests.post", lambda url, formdata, timeout: FakeResponse(html)
    )
...
def test_main_api_error(monkeypatch, test_dict: FullTestDict):
    ...
    monkeypatch.setattr(
        "requests.post",
        lambda x, formdata, timeout: FakeResponse(response, status_code=400),
    )
    assert suzuki.main(word_list) == expected_output
...
def test_get_html_failure(monkeypatch, test_dict: FullTestDict):
    ...
    with pytest.raises(suzuki.SuzukiAPIError) as api_error:
